...
namespace Huobi {
    ...
    public:
        ...
        InputChecker* checkSymbolList(std::list<std::string> symbols) {
            if (symbols.size() == 0) {
                throw SDKException(SDKException::INPUT_ERROR, "[Input] Symbol is mandatory");
            }
        ...
        InputChecker* checkCallback(const std::function<void(const T&) >& callback) {
            ...
            throw SDKException(SDKException::INPUT_ERROR, "no callback");
        }
        ...
        InputChecker* checkDate(const std::string& value, std::string name) {
            ...
            if (!std::regex_match(value.begin(), value.end(), e)) {
                throw SDKException(SDKException::INPUT_ERROR, "The " + name + "is not correct data format");
                return checker();
            }
            return checker();
        }
