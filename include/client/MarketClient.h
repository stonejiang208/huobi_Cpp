...
#include "model//market/MarketTrade.h"
...
#include "req/market/MarketDepthRequest.h"
#include "req/market/MarketTradeRequest.h"
#include "req/market/MarketHistoryTradeRequest.h"

#include "req/market/SubCandlestickRequest.h"
#include "req/market/SubMarketDetailRequest.h"
#include "req/market/SubMarketDepthRequest.h"
#include "req/market/SubMarketTradeRequest.h"
#include "req/market/SubMarketBBORequest.h"
namespace Huobi {
    ...
    struct MarketClient {
        ...
        virtual void subMarketDetail(const SubMarketDetailRequest& request, const std::function<void( const MarketDetailEvent&) > callback) = 0;

        virtual std::vector<MarketTicker> getTickers() = 0;

        virtual MarketDepth getMarketDepth(const MarketDepthRequest& request) = 0;

        virtual void subMarketDepth(const SubMarketDepthRequest& request, const std::function<void( const MarketDepthEvent&) > callback) = 0;

        virtual MarketTrade getMarketTrade(const MarketTradeRequest& request) = 0;

        virtual void subMarketTrade(const SubMarketTradeRequest& request, const std::function<void( const MarketTradeEvent&) > callback) = 0;

        virtual std::vector<MarketTrade> getMarketHistoryTrade(const MarketHistoryTradeRequest& request) = 0;

        virtual void subMarketBBO(const SubMarketBBORequest& request, const std::function<void( const MarketBBOEvent&) > callback) = 0;
        //        /**
